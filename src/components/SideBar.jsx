...
const SideBar = () => {
  ...
  return (
    <>
      <button
        className="md:hidden p-4 text-white bg-cyan-950"
      ...
      <aside
        className={`fixed top-0 left-0 w-full h-full bg-cyan-950 text-white min-h-screen transition-transform transform ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 md:block md:w-64 z-20`}
      >
...
const LinkSidebar = ({ link, children }) => {
  ...
  return (
    <Link
      to={link}
      className={`${
        location.pathname === `${link}` ? `text-cyan-300` : `text-white`
